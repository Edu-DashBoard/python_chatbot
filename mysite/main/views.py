...
def save_user_output(request):
    ...
    if request.method == 'POST':
        user_output='hhhhehhhe'

        if user_input:
            # user_input을 데이터베이스에 저장하거나 다른 작업을 수행
            QA = Question_Answer(Question=user_input,Answer=user_output)
            QA.save()
            
            return render(request, 'main/index2.html',{'user_input': user_input, 'user_output':user_output})  # 사용자 입력을 context에 추가
    return render(request, 'main/index2.html', {'user_input': user_input,'user_output':user_output})  # POST 요청이 아닌 경우 또는 user_input이 없는 경우에 대한 응답 
